<template>
  ...
  <ChatBotQuestion1 v-if="questionList1_1" />
  <div class="questionList message-right">
    <div
...
<script>
import ChatBotQuestion1 from '~/components/ChatBotQuestion1'
export default {
components:{
  ChatBotQuestion1
},
  computed:{
    ...
    dialog(){
      return this.$store.state.chatBot.dialog
    },
    questionList1_1(){
      return this.$store.state.chatBot.questionList1_1
    },
    refresh(){
      return this.$store.state.chatBot.refresh
    }
  },
  watch:{
    toggle(){
  ...
  mounted(){
    ...
    this.$store.state.chatBot.goBack1 = this.$refs.goBack1
  },
  methods:{
    close(b){
      if(!b){
        this.$store.state.chatBot.load = true;
        this.load()
      }
      ...
      this.$store.state.chatBot.load = true
      this.$store.state.chatBot.questionList1_1 = false
    ...
    chosenQuestion(q){
      if(q ==='question1'){
        ...
        setTimeout(()=>{
          this.$store.dispatch('chatBot/questionList1_1')
        },2200)
      }else if(q === 'question2'){
    ...
    scrollControl(){
      return new Promise((resolve)=>{
        this.dialog.scrollTo({top:this.dialog.scrollHeight, behavior: 'smooth'})
        resolve()
      })
    },
    answerShow(answerEl){
